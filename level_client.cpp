...
std::vector<std::vector<std::string>> bfs(CURL* curl, const std::string& start, int depth) {
  ...
  for (int d = 0;  d < depth; d++) {
    ...
    int numThreads;
    if(numNodes < fixed_max){
      numThreads = numNodes;
    }
    else {
      numThreads = fixed_max;
    }

    int nodesPer = numNodes / numThreads;
    int nodeLeftOver = numNodes % numThreads;
    ...
    for(int t = 0; t < numThreads; t++) {
      ...
      int extra = (t < nodeLeftOver) ? 1 : 0;
      int endIndex = startIndex + nodesPer + extra;

      threads.emplace_back([&, startIndex, endIndex, d]() {
        ...
        for(int i = startIndex; i < endIndex; i++) {
          ...
          try {
            ...
            for (const auto& neighbor : get_neighbors(fetch_neighbors(local_curl, s))) {
